...
import { Box, Button, Grid, useTheme, useMediaQuery, 
    CircularProgress, Stepper, Step, StepLabel, Typography, StepContent,
...
import loadButtons from "@components/Button"; // Assuming this is your button component
...
const MemoizedLoadableScene = React.memo(LoadableScene, (prev, next) => {
    // More robust comparison, check for changes in relevant parts of context
    return (
        prev.context.done === next.context.done &&
        JSON.stringify(prev.context.display) === JSON.stringify(next.context.display) &&
...
const StepperMobile = (props) => {
    ...
    const { t }                             = useTranslation(props.source);
    ...
    const memoizedStepperPayload = useCallback((values, indexContext, indexDisplay, context) => {
        try{
    ...
    }, [/* updateContextDone, contexts.length, props.payload */]);
